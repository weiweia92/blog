...
"""
src_alphabet = [chr(ord('a')+i) for i in range(26)]

def get_new_alphabet(src_alphabet, encodestr):
    new_alphabet = []
    for i in encodestr.lower():
        if i not in new_alphabet:
            new_alphabet.append(i)
    for i in src_alphabet:
        if i not in new_alphabet:
            new_alphabet.append(i)
    return new_alphabet

def get_dict(src_alphabet, new_alphabet):
    dic = {}
    for i in range(len(new_alphabet)):
        dic[src_alphabet[i]] = new_alphabet[i]
    return dic


if __name__=='__main__':
    key, encodestr = input(), input()
    new_alphabet = get_new_alphabet(src_alphabet, key)
    dic = get_dict(src_alphabet, new_alphabet)
    newencodestr = ""
    for i in encodestr:
        newencodestr += dic[i]
    print(newencodestr)