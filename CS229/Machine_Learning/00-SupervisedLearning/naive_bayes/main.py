...
def derive_parameter(dictionary, X_train, Y_train, p_y, fi_y_0, fi_y_1):
    error = []
    amount_y_1 = 0
    # x feature vector
    ...
    for row_x, row_y in zip(X_train, Y_train):
        # row_x = ['no', 'no', 'is', 'kallis', 'home', 'ground', 'home', 'town', 'is', 'durban']
        # row_y = 0
        # print(i)
        # the ith train example
        x_temp = feature_handle(row_x) #(372325, 1)  [0,0,0,...,1,0,...,0]
        if row_y:
            amount_y_1 += 1
            x1 = np.add(x1, x_temp)
        else:
    ...
    # laplace smoothing
    fi_y_0 = (x0 + 1)/ (Y_train.shape[0] + 2) #(372325, 1)
    fi_y_1 = (x1 + 1) / (Y_train.shape[0] + 2) #(372325, 1)
...
if __name__=='__main__':
    p_y, fi_y_0, fi_y_1 = derive_parameter(dictionary, X_train, Y_train, p_y, fi_y_0, fi_y_1)
