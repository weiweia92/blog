...
pool 一般放在 conv_layer 之后, 通过池化来降低卷积层输出的特征维度，有效减少网络参数的同时还可以防止过拟合现象,保持一定程度的旋转和平移不变性。**最大池化类似于PCA有效的对数据降维的同时保留关键特征**.pooling可以提高网络对微小位移的容忍能力，缓解卷积层对位置的过度敏感性。提升模型尺度不变性,旋转不变性,抑制噪声降低信息冗余。当特征的信息都具有一定贡献时使用AvgPooling,比如网络深处
...
- 优点：整个函数是以0为中心的，这个特点比sigmod的好。

- 缺点：这两个函数在输入很大或是很小的时候，输出都几乎平滑，梯度很小，不利于权重更新.
...
(2）Xavier 满足x∼U(−a,+a)x 的均匀分布， 其中 $a = \sqrt{3/n}$

(3）MSRA 满足 $x \sim \mathcal{N}(0,\sigma^2)$的高斯分布，其中$\sigma=\sqrt{2/n}$

(4）Uniform 满足min=0,max=1的均匀分布。x∼U(min,max) 等等
...
假设给定： 输入层宽度W，高度H，卷积层尺寸F，步幅为S，填充边框为P，滤波器数量K  
W_out = [(W−F+2P)/S]+1  
H_out = [(H-F+2P)/S]+1  
D_out = K  
